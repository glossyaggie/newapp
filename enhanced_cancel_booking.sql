-- Enhanced cancel_booking RPC function
-- Run this in Supabase SQL Editor

-- Enhanced cancel booking function with comprehensive updates
...
BEGIN
  ...
  IF v_booking_status = 'booked' THEN
    ...
    IF v_booked_count < v_class_capacity THEN
      -- Get the next person on waitlist
      SELECT user_id
      ...
      IF v_waitlist_user_id IS NOT NULL THEN
        UPDATE class_bookings
        SET status = 'booked'
        WHERE class_id = v_class_id AND user_id = v_waitlist_user_id AND status = 'waitlist'
        ORDER BY created_at ASC
        LIMIT 1;
      END IF;
    END IF;
