...
-- Admin function to manually create a pass for testing
CREATE OR REPLACE FUNCTION admin_create_test_pass(
  p_user_email TEXT,
  p_pass_type_name TEXT DEFAULT '5-Class Pack'
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Find pass type by name
  SELECT id INTO v_pass_type_id
  FROM pass_types
  WHERE name = p_pass_type_name AND active = true;
  
  IF NOT FOUND THEN
    RETURN json_build_object(
      'success', false,
      'error', 'pass_type_not_found'
    );
  END IF;
  
  -- Calculate validity period
  SELECT NOW() + (duration_days || ' days')::INTERVAL INTO v_valid_until
  FROM pass_types
  WHERE id = v_pass_type_id;
  ...
  VALUES (
    v_user_id,
    v_pass_type_id,
    (SELECT credits FROM pass_types WHERE id = v_pass_type_id),
    v_valid_from,
    v_valid_until,
    true,
    'active'
  ...
  RETURN json_build_object(
    ...
    'user_id', v_user_id,
    'pass_type', p_pass_type_name,
    'valid_until', v_valid_until
  );
END;
$$;

-- Simple function to create a pass for the specific user (no admin check)
CREATE OR REPLACE FUNCTION create_pass_for_user(
  p_user_id UUID DEFAULT 'cf1e3d02-a237-44e8-8533-9624a98963f8',
  p_pass_type_id UUID DEFAULT '586b6772-8801-4ad9-93fb-4f69fba0037d'
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Calculate validity period
  v_valid_until := NOW() + (v_pass_type.duration_days || ' days')::INTERVAL;
  ...
  RETURN json_build_object(
    ...
    'message', 'Pass created successfully',
    'user_id', p_user_id,
    'pass_type_name', v_pass_type.name,
    'valid_until', v_valid_until
  );
END;
$$;